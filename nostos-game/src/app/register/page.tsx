...
export default function RegisterPage() {
  ...
  useEffect(() => {
    if (!shipName.trim()) { setShipTakenError(null); return; }
    const id = setTimeout(async () => {
      setIsCheckingShip(true);
      try {
        const res = await fetch(`/api/check-ship?ship=${encodeURIComponent(shipName)}`);
        const data = await res.json();
        setShipTakenError(data.taken ? "That ship is already sailing these waters." : null);
      } catch { /* ignore */ } finally { setIsCheckingShip(false); }
    }, 500);
  ...
  if (state.success) {
    return (
      <main className="min-h-screen text-parchment font-serif bg-ink flex flex-col items-center justify-center p-6 selection:bg-gold selection:text-ink relative overflow-hidden">
        <div className="fixed inset-0 opacity-40 pointer-events-none z-0"><OceanCanvas /></div>
  ...
  return (
    <main className="min-h-screen text-parchment font-serif bg-ink selection:bg-gold selection:text-ink flex flex-col items-center justify-center p-6 md:p-12 relative overflow-hidden">
      <div className="fixed inset-0 opacity-35 pointer-events-none z-0"><OceanCanvas /></div>
      ...
      <div className="relative z-10 w-full max-w-xl">
        <div className="text-center mb-12 space-y-3">
          ...
          <div className="max-w-xs mx-auto"><GreekKeyLine /></div>
        </div>
          ...
          <form action={formAction} key={state.timestamp || "init"} className="space-y-12">
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-2 relative">
                ...
                {effectiveShipError && <p className="text-danger italic text-sm mt-1">{effectiveShipError}</p>}
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                {state.errors?.password && <p className="text-danger italic text-sm mt-1">{state.errors.password}</p>}
              </div>
            </div>
